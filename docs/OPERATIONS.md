...
Docker Compose stack behind Caddy, and an `aws-*` Makefile target for each command below that you
...
On the deployed VM the equivalent is [`make aws-verify-db`](#75-the-database), and it asserts
...
[`infra/scripts/backup-to-s3.sh`](../infra/scripts/backup-to-s3.sh), which pushes to an S3 bucket in
`eu-central-1` using the EC2 instance profile as its credential.

It runs `pg_dump` against Neon's **direct** endpoint over TLS, from inside a throwaway, pinned
...
**One caveat worth naming.** Neon runs on AWS, so the backup bucket is with the same *provider* as
the database rather than a different one. It is still a different account, a different service and a
different credential, which is what the principle above is asking for — but it is not the provider
diversity it might sound like. A practice that wants that needs a third copy somewhere else
...
One VM — a `t3.small` in `eu-central-1` (2 vCPU, 2 GiB, a 32 GiB gp3 volume) — running the
same `docker-compose.yml` as everywhere else with
[`docker-compose.aws.yml`](../infra/docker/docker-compose.aws.yml) layered on top. That override
...
Provisioning and the first deploy are in [`docs/deploy/AWS.md`](deploy/AWS.md). This section is what
you do afterwards, and almost none of it is AWS-specific: it is SSH and `docker compose`.

Set the host once and every `aws-*` target below works:
...
it, since nothing builds any more), or `make aws-restart SERVICE=web`. To run only the migration,
without a full deploy: `make aws-migrate`.
...
```bash
make aws-logs                          # engine, followed
make aws-logs SERVICE=web
make aws-logs-caddy                    # TLS and certificate problems live here
```

Piping through `jq` needs the pipe to run locally, so for anything analytical, pull the lines down:

```bash
ssh $AWS_HOST 'cd /opt/azmoth/repo && sudo docker compose \
...
```bash
ssh $AWS_HOST 'sudo du -sh /var/lib/docker/containers/*/*-json.log | sort -h | tail'
```

### 7.4 Restarting

```bash
make aws-ps                            # what is running, and is it healthy
make aws-restart SERVICE=web
make aws-restart SERVICE=engine
make aws-restart SERVICE=caddy         # after editing the Caddyfile — see below
```
...
```bash
ssh $AWS_HOST 'df -h / && docker system df'
ssh $AWS_HOST 'sudo docker system prune -af --filter until=168h'   # keeps the last week's images
```
...
duplicates a week. (`azmoth-postgres-data` is not on this box at all — the aws override profiles
...
It is not on this box, so nothing here is a `docker compose exec`. `make aws-psql` runs `psql` in
...
```bash
make aws-verify-db                     # on Neon? on the DIRECT endpoint? at schema head?
make aws-psql                          # an interactive session, in postgres:17-alpine
make aws-migrate                       # alembic upgrade head, without a full deploy
```

`make aws-verify-db` answers three questions rather than two. It prints the backend, whether the
...
**There is no SSH tunnel to keep alive.** A GUI — DataGrip, TablePlus, `psql` on your laptop —
connects to the Neon host **directly** with the connection string from the Neon console, over TLS.
There is no port on the box that ever needs to be reachable for this.
...
to every table. The aws override reassigns its profile with `!override` — which *replaces* the
...
The backup job — [`backup-to-s3.sh`](../infra/scripts/backup-to-s3.sh) — no longer wraps
...
it to S3 in the same region. [§ 2](#2-backups) has the reasoning for the container and
...
- **The VM holds no *storage* credential.** It is the EC2 instance profile
  ...
  metadata service. There is no key on disk to steal from beside the dumps it would decrypt.

  Two consequences, both deliberate and both surprising the first time:
...
- **The Neon connection string, however, IS a long-lived credential, and it does live on the box** —
  ...
  instance profile does not get rid of, and it is what this script's `pg_dump` uses too. Rotating it
  ...
  the credentials that can read the database, and without them the encrypted dumps in S3
...
`sudo apt-get install -y age` used to be a line in that block, and installing the AWS CLI by hand
...
binary is one more reason for later never to arrive. `postgresql-client` is deliberately
...
backups it just wrote. This is enforced rather than merely intended — the instance profile has
no `s3:DeleteObject`. Expire them with a lifecycle rule instead, which the VM's role does not permit
it to change.

**Mind that the bucket is versioned when you write that rule.** A rule that expires *current*
...
```bash
# from your LAPTOP; the VM has no s3:ListBucket
...
It *has* to run from your laptop for a second reason: the VM's instance profile has no
`s3:ListBucket`, so the box cannot even find the file. That is the policy working, not an obstacle to
route around.

`make aws-check-db` is not a restore drill and does not pretend to be. It is the database section
of the pre-flight — reachable, right endpoints, schema at head. A real drill needs
...
[`backup-to-s3.sh`](../infra/scripts/backup-to-s3.sh) warns about, and it is worth being
careful with twice.
...
recognise. If it says the schema is behind: `make aws-migrate`.

### 7.8 Cost and the budget
...
**There is no spending cap on an AWS account.** A runaway resource keeps billing a card until
somebody notices, rather than stopping on its own. The budget alert from
[RUNBOOK.md § 1.3](deploy/RUNBOOK.md#13-set-a-budget-alert-before-you-create-anything) is not
optional.
...
are visible in the Neon console before they happen.

### 7.9 When it is broken

| What you see | Look here first |
|---|---|
| Browser warns about the certificate | `make aws-logs-caddy`. Caddy serves a self-signed certificate while ACME is failing. Usually DNS not resolving to this VM, or port 80 closed. |
...
| A container killed with exit 137 | OOM on a 2 GiB box. `free -m` — the 4 GiB swapfile should be there; re-run `infra/aws/provision.sh` for this box if it is not. Nothing is built here any more, so this is a runtime spike (usually a Soufflé solve), not a build. |
| Containers restarting, no obvious cause | `df -h /`. A full disk looks like everything failing at once. |
| `502` from Caddy | The backend is not healthy yet. `make aws-ps`, then that service's logs. |
| Port 8000 answers from outside | Stop. The deploy did not use `docker-compose.aws.yml`. Anyone can write to the audit log as any practice until it is closed. |
