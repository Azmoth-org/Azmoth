...
| `UNKNOWN_ZIFFER` | 422 | **Every** GOÄ position in the delivery is absent from the loaded catalog. `POST /padnext/audit` only — the batch path reports a mismatched file as one low-coverage report among many rather than failing the upload. `details.unknown_ziffern` lists them, `details.catalog_version` names what they were checked against. | Almost always the wrong catalog edition, not a typo — check which GOÄ the delivery was coded against. A delivery with only *some* unknown positions is **not** an error: those are audited, marked `unknown_ziffer` and counted in the `unconfirmed` bucket. Percentage Zuschläge (Nummern 441 and 5298) do **not** count towards this refusal and never trigger it: § 5 GOÄ gives them no Punktzahl, so no edition holds one and their absence proves nothing about which edition the sender used — they are audited and marked `surcharge_not_modelled`. |
...
| `UNSUPPORTED_FILE_EXTENSION` | 400 | The uploaded file's **name** ends in something the endpoint does not take, or carries an executable suffix anywhere in it (`invoice.php.pdf`). Checked before the bytes are, and it is the *name* check — `UNSUPPORTED_INPUT_FORMAT` is the separate one decided by the leading bytes. `details.rejected_extension` is what was sent, `details.accepted` is the list for that endpoint. | Send the file under its real extension: `.xml` or `.padx` for a delivery, `.zip` for a bulk archive. If the bytes are right and only the name is wrong, renaming is the whole fix — which is exactly why this is not `UNSUPPORTED_INPUT_FORMAT`. |
| `INVALID_FILENAME` | 400 | The multipart part's filename is unusable: absent, longer than 255 characters, or containing a path separator or a NUL byte. Almost never a format mistake — a separator here means a caller sent a path where a bare name belongs. | Send the base filename only, not a path. Note that the name is metadata: it is echoed into the report's `source_name` and into log lines, and nothing branches on it. |
| `EMPTY_FILE` | 400 | A multipart part arrived with a filename and zero bytes. Distinct from `EMPTY_REQUEST_BODY`, which describes a whole request: in a batch upload what matters is *which* file was empty, and `details.filename` names it. | Fix the client's multipart assembly — an empty part is a client bug rather than a delivery. |
...
| `SESSION_TOKEN_REQUIRED` | 401 | `POST /api/v1/rules/{rule_id}/review` carried no `Authorization: Bearer …` header. `details.header` names it. This is the one endpoint on the web tier's surface that verifies a Better Auth session itself — see [`app/api/session_auth.py`](../apps/engine/app/api/session_auth.py). | Send the session token; the web proxy (`apps/web/lib/engine.ts`) already does this on every call. A direct caller has to mint one itself via Better Auth's `jwt()` plugin. |
| `SESSION_TOKEN_INVALID` | 401 | The bearer token failed to verify: bad signature, expired, wrong issuer/audience, or signed by a key this engine does not recognise. **All of these answer identically**, for the same reason `API_KEY_INVALID` does. | Reload the page and try again — a reviewer's session token is minted per request and expires in minutes. Deterministic for a given token; a fresh one succeeds. |
...
| `QUOTA_EXCEEDED` | 429 · `Retry-After` | The practice has audited everything its plan includes for the current billing period, and the plan does **not** permit going over (`allow_overage` false — the free tier). Raised on both audit paths, and for a bulk upload it is raised for the archive as a whole rather than part-way through it. `details.quota`, `details.used`, `details.requested`, `details.plan_code`, `details.subscription_tier`. | Upgrade the plan (`POST /api/v1/billing/upgrade`), or wait for the period to roll — `Retry-After` is the seconds until it does, which is days rather than seconds. **Do not treat this like `RATE_LIMIT_EXCEEDED`:** both are `429`, but a rate limit clears by waiting a minute and this clears by waiting up to a month or by changing plan, so a backoff loop on it runs for weeks. `X-Quota-Limit` / `-Remaining` / `-Reset` are on every **successful** audit response too. A plan with overage allowed is never refused here — it is charged instead, and `GET /api/v1/billing/usage` shows what has accrued. See [`BILLING.md`](BILLING.md). |
