...
> {
  ...
  modifiers?: Modifiers
  /** Collision strategy. Defaults to pointerWithin — the default rectIntersection
   *  double-shifts with the live-preview layout (item moves into the target
   *  column during dragOver, then the drag transform pushes the collision
   *  rect one column further), landing drops on the wrong column. */
...
function KanbanItem({
  ...
  const isOverlay = useContext(IsOverlayContext)
  // Gate dnd-kit's a11y attributes behind mount: they carry a module-level
  // counter (aria-describedby="DndDescribedBy-N") that advances differently
  // during SSR → hydration mismatch on every board render.
